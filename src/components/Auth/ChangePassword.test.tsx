...
describe('ChangePassword', () => {
  ...
  it('renders the change password form', () => {
    ...
    expect(
      screen.getByText(enTranslations.auth.changePassword.currentPassword),
    ).toBeInTheDocument();
    expect(screen.getByText(enTranslations.auth.changePassword.newPassword)).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: enTranslations.auth.changePassword.button.default }),
    ).toBeInTheDocument();
  });
  ...
  it('handles successful password change', async () => {
    ...
    // Fill in the form
    fireEvent.change(
      screen.getByPlaceholderText(enTranslations.auth.changePassword.currentPasswordPlaceholder),
      {
        target: { value: 'current-password' },
      },
    );
    fireEvent.change(
      screen.getByPlaceholderText(enTranslations.auth.changePassword.newPasswordPlaceholder),
      {
    ...
    const callArgs = (supabase.auth.signInWithPassword as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
  ...
  it('handles incorrect current password', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(enTranslations.auth.changePassword.error.title),
        ).toBeInTheDocument();
      },
      { timeout: 2000 },
    );
