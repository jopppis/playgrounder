...
describe('SignUp', () => {
  ...
  it('renders the sign up form', () => {
    ...
    expect(
      screen.getByRole('heading', { name: enTranslations.auth.signUp.title }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole('button', { name: enTranslations.auth.signUp.button.default }),
    ).toBeInTheDocument();
  });
  ...
  it('shows loading state during sign up', async () => {
    ...
    await waitFor(
      () => {
        ...
        const loadingButton = buttons.find((button) => button.textContent?.includes(loadingText));
