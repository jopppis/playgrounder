  ...
  ({
    ...
    useEffect(() => {
      const checkVisibility = async () => {
        ...
        const shouldShow =
          !hasVisiblePlaygrounds && playgrounds.length > 0 && filteredPlaygrounds.length > 0;
    ...
    return (
      <Box
        ...
        border="0px"
      >
        <Text mb={3} color={noPlaygroundsAtAll ? 'gray.900' : 'gray.700'}>
          {noPlaygroundsAtAll ? t('map.noPlaygroundsMatchFilters') : t('map.noVisiblePlaygrounds')}
        </Text>
        <Box
          display="flex"
          flexDirection={noPlaygroundsAtAll ? 'column' : 'row'}
          gap={2}
